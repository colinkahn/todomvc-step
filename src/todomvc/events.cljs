...
(defn check-and-throw
  ...
  [a-spec step]
  (if-not (s/valid? a-spec step)
    (throw (ex-info (str "spec check failed: " (s/explain-str a-spec step)) {}))
    (let [[state [event [state']]] (s/conform a-spec step)]
  ...
  (->interceptor
    ...
    (fn [context]
      (let [db' (get-in context [:effects :db])
        ...
        (check-and-throw :todomvc.db/step [db event db'])
        context))))
